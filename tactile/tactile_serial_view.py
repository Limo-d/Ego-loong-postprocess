...
协议（ASCII 文本，一行/帧，以 \\n 或 \\r\\n 结尾）：
  ...
  串口输出以 TOUCH 为准：收到有效 TOUCH 行即输出触觉帧。
  SOLVED 为可选附加信息；含 MISS 或未配对时不影响 TOUCH 输出。
...
可视化入口（本文件）：HandTactileViewer 2D 脉冲点；--viz-3d 使用 MANO 3D 网格热力图。
串口解析与数据结构亦可在本模块 import；解耦/滑移等脚本默认 import tactile_serial_reader。
"""
...
class TactileGloveReader:
    ...
    def _joint_angles_from_solved(
        self,
        solved: Optional[SolvedKinematics],
    ) -> Optional[np.ndarray]:
        """尽力从 SOLVED 提取关节角；MISS 或不完整时返回 None，不影响 TOUCH。"""
        if solved is None or not solved.is_complete:
            return None
        try:
            return flex_joint_angles_from_mdeg(solved.angles_mdeg)
        except ValueError:
            return None
    ...
    def _finalize_frame(
        ...
        joint_angles = self._joint_angles_from_solved(solved)
        if (
            joint_angles is not None
            and solved is not None
            and not self._solved_ready_announced
        ):
            self._solved_ready_announced = True
            if self.debug:
                print(
                    f"[debug] SOLVED 已就绪（touch_seq={sequence}, "
                    f"solved_seq={solved.sequence}），附带关节角"
                )
    ...
    def _process_line(self, line: str, raw: bytes) -> Optional[TactileFrame]:
        ...
        # 独立 SOLVED 行（含 MISS）忽略，不阻塞 TOUCH
        return None
    ...
    def _timeout_diagnostics(self) -> str:
        ...
        if self._bytes_read == 0:
            ...
            lines.append(
                "有数据但未找到有效 TOUCH 行：可能波特率不对或行未以换行结束"
            )
        return "\n  ".join(lines)
    ...
    def frames(self) -> Generator[TactileFrame, None, None]:
        while True:
            yield self.read_frame(timeout_s=5.0)
...
def run_stream(
    ...
    *,
    ...
    try:
        ...
        print(e)
        print("建议: python tactile_serial_view.py --port", port, "--debug")
...
def run_visualizer_3d(
    ...
    *,
    ...
    if not mano_models_available():
        ...
        if not (_root / "mano_v1_2").is_dir() and (_root.parent / "mano_v1_2").is_dir():
            _root = _root.parent
        raise SystemExit(
            "3D 可视化需要 MANO 模型文件。\n"
            f"  1. 从 https://mano.is.tue.mpg.de 下载 MANO_LEFT.pkl / MANO_RIGHT.pkl\n"
            f"  2. 放入 {_root / 'mano_v1_2' / 'models'}\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="左手触觉手套串口读取与可视化（68 点）")
