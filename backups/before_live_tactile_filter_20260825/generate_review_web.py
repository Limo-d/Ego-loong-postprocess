...
def load_rows(traj_path: Path) -> tuple[List[Dict[str, Any]], List[np.ndarray], List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
    ...
    for line in traj_path.read_text(encoding="utf-8").splitlines():
        ...
        for side, key in (("left", "hand_l"), ("right", "hand_r")):
            hand = hand_sources.get(side) or {}
            optimized = hand.get("optimized_trajectory") or {}
            glove = hand.get("glove") or {}
            world = optimized.get("kpts_3d_world_m_optimized") or glove.get("kpts_3d_world_m")
            ...
            camera = optimized.get("kpts_3d_camera_m_optimized") or glove.get("kpts_3d_camera_m")
...
def build_chart_data(traj_path: Path, frame_times: List[float]) -> List[Dict[str, Any]]:
    ...
    for row_index, line in enumerate(lines):
        ...
        for side, prefix in (("left", "l"), ("right", "r")):
            ...
            optimized = hand.get("optimized_trajectory") or {}
            glove = hand.get("glove") or {}
            for field, fallback_field, key_prefix in (
                ("kpts_3d_world_m_optimized", "kpts_3d_world_m", prefix),
                ("kpts_3d_camera_m_optimized", "kpts_3d_camera_m", f"c{prefix}"),
            ):
                pts = optimized.get(field) or glove.get(fallback_field)
                if pts is None:
                    continue
...
def build_trajectory_web_data(rows: List[Dict[str, Any]], all_points: List[np.ndarray], hand_display_scale: float = 3.0) -> Dict[str, Any]:
    def build_coordinate_data(suffix: str, coordinate_points: List[np.ndarray]) -> Dict[str, Any]:
        ...
        # roll=pi keeps fingers pointing upward in the default review view, but
        # it can also mirror screen X. Preserve the physical/RGB convention:
        # hand_l must appear to the left of hand_r in the initial view.
        screen_x_sign = 1.0
        wrist_separations = []
        for item in frames:
            left = item.get("hand_l")
            right = item.get("hand_r")
            if left is not None and right is not None:
                wrist_separations.append(
                    np.asarray(right[0], dtype=np.float64)
                    - np.asarray(left[0], dtype=np.float64)
                )
        if wrist_separations:
            delta = np.median(np.stack(wrist_separations, axis=0), axis=0)
            projected_dx = -(
                np.cos(default_yaw) * float(delta[0])
                - np.sin(default_yaw) * float(delta[1])
            )
            if projected_dx < 0.0:
                screen_x_sign = -1.0
        return {
