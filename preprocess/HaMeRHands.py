...
Description:
    ...
    Produces Hands-compatible output so it can be directly swapped with MPS hand
    tracking or MediaPipe-based tracking for ablation studies.

Pipeline:
    1. Read RGB frames from the existing Cam sequence
    ...
    5. Remap HaMeR 21-point ordering (same as MediaPipe after reordering) to MPS ordering
    ...
    9. Apply temporal smoothing (Savitzky-Golay + EMA) via HandsOptimizer
...
from preprocess.CamTypes import Cam
from preprocess.HandsTypes import (
...
from preprocess.HandsOptimizer import HandsOptimizer
from preprocess.HandsOps import HandsOps


def _patch_numpy_legacy_aliases() -> None:
    """
    chumpy/MANO used by HaMeR still imports legacy numpy aliases removed in
    numpy 2.x. Provide them locally before importing hamer so newer envs work.
    """
    aliases = {
        "int": int,
        "float": float,
...
# Prefer the checked-in HaMeR assets for reproducibility. Override with
# HAMER_CACHE_DIR if a different asset root is needed.
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
DEFAULT_HAMER_CACHE_DIR = os.path.join(PROJECT_ROOT, "hamer", "_DATA")
HAMER_CACHE_DIR = os.path.expanduser(os.environ.get("HAMER_CACHE_DIR", DEFAULT_HAMER_CACHE_DIR))
HAMER_CKPT_FILENAME = "new_hamer_weights.ckpt"
...
def _ensure_hamer_ckpts() -> str:
    ...
    Returns:
        Path to the configured HaMeR checkpoint.
    """
    cache_dir = HAMER_CACHE_DIR

    ckpt_dir = os.path.join(cache_dir, "hamer_ckpts", "checkpoints")
    local_ckpt = os.path.join(ckpt_dir, HAMER_CKPT_FILENAME)
    fallback_ckpt = os.path.join(ckpt_dir, "hamer.ckpt")
    for candidate in (local_ckpt, fallback_ckpt):
    ...
    try:
        ...
        print(f"[HaMeR] Please manually place {HAMER_CKPT_FILENAME} at: {local_ckpt}")
...
# ==================================================================
# Legacy HaMeR/MediaPipe → MPS index mapping.
# Disabled by request: HaMeR outputs are now saved in native order.
# ==================================================================
...
def remap_mp_to_aria(kpts_mp_21: np.ndarray) -> np.ndarray:
    """
    Keep 21 HaMeR/MediaPipe keypoints in their original order.

    Args:
        kpts_mp_21: (21, 3) keypoints in HaMeR/MediaPipe ordering.

    Returns:
        (21, 3) keypoints in HaMeR/MediaPipe ordering.
    """
    # Reordering to MPS is intentionally disabled; downstream JSON keeps
    # HaMeR/MediaPipe's native 0..20 landmark convention.
    return kpts_mp_21.copy()
...
class HaMeRModel:
    ...
    def __init__(self, device: str = "cuda"):
        ...
        self.cache_dir = HAMER_CACHE_DIR
        self.mano_path = os.path.join(HAMER_CACHE_DIR, "data", "mano", "MANO_RIGHT.pkl")
        self.mano_mean_params_path = os.path.join(HAMER_CACHE_DIR, "data", "mano_mean_params.npz")
        ...
        try:
            ...
            print(f"[HaMeR] Checkpoint: {self.checkpoint_path}")
            print(f"[HaMeR] MANO: {self.mano_path}")
        except Exception as e:
    ...
    def predict_from_crop(
        ...
        try:
            ...
            vertices_cam = pred_vertices + pred_cam_t_full[np.newaxis, :]
...
class ViTPoseHandDetector:
    ...
    # Default to the small model: an 8GB GPU cannot reliably keep ViTPose-H,
    # YOLO, and HaMeR resident at the same time.
    VITPOSE_VARIANTS = [
    ...
    def __init__(self, device: str = "cuda", variant: str = "s"):
        from huggingface_hub import hf_hub_download
        from easy_ViTPose import VitInference

        variant = (variant or "s").lower()
        if variant not in {"s", "b", "l", "h"}:
            raise ValueError(f"Unknown ViTPose variant: {variant}")

        # Download ViTPose checkpoint. Try the requested variant first, then
        # smaller/larger fallbacks if the checkpoint is unavailable.
        variants = sorted(
            self.VITPOSE_VARIANTS,
            key=lambda item: 0 if item[0] == variant else 1,
        )
        vitpose_model = None
        model_name = None
        for candidate, hf_path in variants:
            try:
...
class HaMeRHandsGenerator:
    """
    Generates Hands-compatible hand tracking data using HaMeR (Hand Mesh Recovery).
    ...
    Produces the same Hands data structure and JSON format for seamless integration.
    """
    ...
    def __init__(
        ...
        cfg_path: str,
        aria_cam: Cam,
        detector: str = "vitpose",
        vitpose_variant: str = "s",
        vitpose_device: str = "cuda",
        hamer_device: str = "cuda",
    ):
        """
        Args:
            mps_path: Root data directory (e.g., data/mps_serve_bread_000_vrs).
            cfg_path: Path to Hands.yaml config (reuses same config).
        ...
        if detector == "mediapipe":
            ...
            try:
                self.detector = ViTPoseHandDetector(device=vitpose_device, variant=vitpose_variant)
        ...
        # Stage 2: HaMeR model
        self.hamer_model = HaMeRModel(device=hamer_device)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    def get_aria_hands(self) -> Hands:
        """
        Full pipeline: extract -> clean -> optimize -> return Hands.
        """
        aria_hands = Hands(mps_path=self.mps_path)
        ...
        for i, cam_data in enumerate(tqdm(self.aria_cam.cam, total=len(self.aria_cam),
            ...
            if img_bgr is None:
                ...
                if os.path.isfile(img_path):
                    ...
                    aria_hands.hands.append(HandsData(cam_data.idx, cam_data.ts))
            ...
            try:
                ...
                if "out of memory" not in str(e).lower():
                    raise
                print("[HaMeR] WARNING: detector CUDA OOM; retrying this run with MediaPipe detector")
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                self.detector = MediaPipeHandDetector()
                self._detector_name = "MediaPipe"
        ...
        # and have more intermittent gaps than MPS, so:
        ...
        optimizer = HandsOptimizer(self.cfg, dt)
        ...
        HandsOps.print_summary_and_eval(aria_hands)
    ...
    # ==================================================================
    # Build HandData from camera-frame keypoints
    # ==================================================================

    def _build_hand_data(
        self,
        kpts_cam_aria: np.ndarray,   # (21, 3) HaMeR/MediaPipe ordering, camera frame
        kpts_2d_aria: np.ndarray,    # (21, 2) HaMeR/MediaPipe ordering, pixel coords
        confidence: float,
        c2w: np.ndarray,
        k: np.ndarray,
        h_img: int, w_img: int,
        ...
        """Build HandData from camera-frame 21 keypoints."""

        # HaMeR/MediaPipe order: 0 wrist, 4 thumb tip, 5 index MCP,
        # 8 index tip, 9 middle MCP. There is no palm-center landmark.
        wrist_pos_cam = kpts_cam_aria[0]
        palm_center_cam = (kpts_cam_aria[0] + kpts_cam_aria[5] + kpts_cam_aria[9]) / 3.0
        index_mcp_cam = kpts_cam_aria[5]
        middle_mcp_cam = kpts_cam_aria[9]
        ...
        # thumb tip (MP 4) vs index tip (MP 8), normalized by palm size
        thumb_tip = kpts_cam_aria[4]
        index_tip = kpts_cam_aria[8]
        wrist = kpts_cam_aria[0]
        mid_mcp = kpts_cam_aria[9]
        ...
        # HandsJointAngles expects hand ordering; native HaMeR/MediaPipe
        # order is kept in the saved keypoints, so skip angle export here.
    ...
    # ==================================================================
    # Velocity & Midpoint computation (mirrors Hands.py logic)
    # ==================================================================
    ...
    # ==================================================================
    # Temporal cleaning (mirrors Hands.py logic)
    # ==================================================================

    def _filter_by_confidence(self, aria_hands: Hands, conf_th: float = 0.3) -> None:
    ...
    def _suppress_short_hands(self, aria_hands: Hands, min_frames: int = 5) -> None:
        for attr in ["hand_r", "hand_l"]:
    ...
    def _interpolate_hand_trajectories(self, aria_hands: Hands, max_gap: int = 3) -> None:
    ...
    def _smooth_grasp_detection(self, aria_hands: Hands, size: int = 5) -> None:
...
def run_hamer_hands(mps_path: str, cfg_path: str, aria_cam=None,
                    export_video: bool = False, export_gif: bool = False,
                    detector: str = "vitpose", vitpose_variant: str = "s",
                    vitpose_device: str = "cuda", hamer_device: str = "cuda") -> Hands:
    """
    Entry point for Preprocess.py integration.

    Args:
        mps_path: Root data directory.
        cfg_path: Path to Hands.yaml config.
        aria_cam: Optional pre-built Cam object. If None, reconstructs from disk.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg_path", type=str, default="./cfg/preprocess/base/Hands.yaml")
    parser.add_argument("--export_video", action="store_true")
    parser.add_argument("--export_gif", action="store_true")
    parser.add_argument("--detector", choices=["vitpose", "mediapipe"], default="vitpose")
    parser.add_argument("--vitpose_variant", choices=["s", "b", "l", "h"], default="s")
    parser.add_argument("--vitpose_device", default="cuda")
    parser.add_argument("--hamer_device", default="cuda")
    ...
    run_hamer_hands(args.mps_path, args.cfg_path,
                    ...
                    hamer_device=args.hamer_device)
