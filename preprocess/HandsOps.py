...
"""
====================================================================================================
Hand Tracking Visualization & Analysis Operations (HandsOps.py)
====================================================================================================

Description:
    This module provides a collection of static utility methods for visualizing and analyzing
    processed hand tracking data from Project Machine Perception Services (MPS).
    It operates on pre-processed HandsData / Hands structures and covers two major areas:
...
Technical Specifics:
    - All optimized ("Opt") data refers to trajectories smoothed via Savitzky-Golay / EMA
      filtering applied upstream in the pipeline (Hands.py).
...
try:
    ...
    class _FallbackHandTracking:
        # Hands-compatible 21-keypoint order used in this repo:
        # 0 thumb tip, 1 index tip, 2 middle tip, 3 ring tip, 4 pinky tip,
        # 5 wrist, 6-7 thumb, 8-10 index, 11-13 middle,
        # 14-16 ring, 17-19 pinky, 20 palm center.
    ...
    class _FallbackMps:
        hand_tracking = _FallbackHandTracking()

    mps = _FallbackMps()

from utils.utils_vis import draw_glass_rect
from preprocess.HandsTypes import HandsData, Hands
...
class HandsOps:
    ...
    def _collect_series_single_hand(aria_hands: Hands, is_right: bool) -> Dict:
        """
        Extract all per-frame time-series data for one hand from an Hands sequence,
        ...
        Args:
            aria_hands (Hands): Full hand tracking sequence with per-frame HandsData.
        ...
        for i, frame_data in enumerate(aria_hands.hands):
            ...
            uses_native_mp_order = not (h.joint_angles and h.joint_angles.data)
            thumb_tip_idx, index_tip_idx = (4, 8) if uses_native_mp_order else (0, 1)

            if h.hand_keypoints_3d is not None and len(h.hand_keypoints_3d) > index_tip_idx:
                dist_raw[i] = np.linalg.norm(
                    h.hand_keypoints_3d[thumb_tip_idx] - h.hand_keypoints_3d[index_tip_idx]
                )
            ...
            # 2D midpoint: average thumb tip and index tip in the active keypoint order.
            if h.hand_keypoints_2d is not None and len(h.hand_keypoints_2d) > index_tip_idx:
                thumb_2d = h.hand_keypoints_2d[thumb_tip_idx]
                index_2d = h.hand_keypoints_2d[index_tip_idx]
                if not (np.allclose(thumb_2d, 0) or np.allclose(index_2d, 0)):
                    mid2d_raw[i, :] = (thumb_2d + index_2d) / 2.0
    ...
    def save_hands_analysis_plots_two(
        ...
        Args:
            aria_hands (Hands): Full sequence hand tracking data.
    ...
    def _draw_aria_hand_panel(
        ...
        Args:
            ...
            hand     (Any)       : HandData object, or None if hand not detected.
    ...
    def _draw_opt_wrist_thumb_index_only(
        ...
        Args:
            img           (np.ndarray): BGR image to draw on, shape (H, W, 3).
            aria_hand_data (Any)      : HandData with optimized world-space fields:
    ...
    def draw_aria_hands_panel(
        ...
        Args:
            ...
            aria_hands_data (HandsData): Container with hand_r and hand_l fields.
            opt_v_limit (float)
    ...
    def _draw_advanced_hud(
        ...
        Args:
            img  (np.ndarray): BGR image to draw on, shape (H, W, 3).
            hand (Any)       : HandData with optimized world fields:
                                 wrist_pose_opt_world, thumb/index_translation_opt_world,
    ...
    def draw_aria_hands_skeleton(
        ...
        for hand in [aria_hands_data.hand_r, aria_hands_data.hand_l]:
            if hand is not None:
                ...
                img = HandsOps._draw_axis(img, np.linalg.inv(c2w) @ hand.midpoint_pose_opt_world, k, d)
                if full_skeleton:
