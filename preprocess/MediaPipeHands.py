...
Description:
    Image-based hand detection using Google's MediaPipe Hands. Produces Hands-compatible
    output so it can be directly swapped with MPS hand tracking for ablation studies.
...
Pipeline:
    ...
    4. Remap MediaPipe 21-point ordering to MPS 21-point ordering
    ...
    8. Apply temporal smoothing (Savitzky-Golay + EMA) via HandsOptimizer
...
# ==================================================================
# Legacy MediaPipe → MPS index mapping.
# Disabled by request: MediaPipe outputs are now saved in native order.
# ==================================================================
# MediaPipe ordering:
...
def remap_mp_to_aria(kpts_mp_21: np.ndarray) -> np.ndarray:
    """
    Keep 21 MediaPipe keypoints in their original order.

    Args:
        kpts_mp_21: (21, 3) keypoints in MediaPipe ordering.

    Returns:
        (21, 3) keypoints in MediaPipe ordering.
    """
    # Reordering to MPS is intentionally disabled; downstream JSON keeps
    # MediaPipe's native 0..20 landmark convention.
    return kpts_mp_21.copy()


class MediaPipeHandsGenerator:
    """
    Generates Hands-compatible hand tracking data using Google MediaPipe Hands.
    Produces the same Hands data structure and JSON format for seamless integration.
    """

    def __init__(self, mps_path: str, cfg_path: str, aria_cam: Cam):
        """
        Args:
    ...
    def _build_hand_data(
        self,
        kpts_cam_aria: np.ndarray,   # (21, 3) MediaPipe ordering, camera frame
        kpts_2d_aria: np.ndarray,    # (21, 2) MediaPipe ordering, pixel coords
        confidence: float,
        c2w: np.ndarray,
        k: np.ndarray,
        h_img: int, w_img: int,
        ...
        # MediaPipe order: 0 wrist, 4 thumb tip, 5 index MCP, 8 index tip,
        # 9 middle MCP. There is no palm-center landmark in native order.
        ...
        # Grasp detection: thumb tip (MP 4) vs index tip (MP 8)
        # Normalize by palm size (wrist→middle_MCP) to make threshold scale-invariant.
        # MediaPipe 3D recovery produces systematically smaller distances than MPS
        ...
        if palm_size > 0.01:
            ...
            # MPS: mean ratio ≈ 1.04, 63% below 1.0.  MediaPipe produces
        ...
        # HandsJointAngles expects hand ordering; native MediaPipe order is
        # kept in the saved keypoints, so skip angle export here.
...
def _build_aria_cam_from_disk(mps_path: str):
    ...
    Returns an Cam with cam[] having img, k, c2w per frame.
    """
    from preprocess.CamTypes import Cam, CamData
    import cv2
    ...
    for fn in frame_dirs:
        ...
        img = cv2.imread(rgb_path)  # Keep as BGR to match Cam convention
...
def run_mediapipe_hands(mps_path: str, cfg_path: str, aria_cam=None,
    ...
    # ── Visualization video (skeleton + HUD overlay, same style as Hands) ──
