...
def build_chart_data(traj_path: Path, frame_times: List[float]) -> List[Dict[str, float]]:
    ...
    for row_index, line in enumerate(lines):
        ...
        pts = (row.get("glove") or {}).get("kpts_3d_camera_m")
        if pts is None:
            continue
...
def write_html(session: Path, chart_rows: List[Dict[str, float]], trajectory_data: Dict[str, Any], frame_times: List[float], fps: float = 30.0, output_subdir: str = "web") -> Path:
...
const DATA={chart_json}; const TRAJ={traj_json}; const FRAME_TIMES={frame_times_json}; const DURATION={duration:.6f}; const FPS={fps:.6f}; const FRAME_COUNT={frames}; const TACTILE_CANVAS_BG="{TACTILE_CANVAS_BG}"; const TRAJECTORY_CANVAS_BG="{TRAJECTORY_CANVAS_BG}";
...
function resizeOne(c){{const r=c.getBoundingClientRect(),d=window.devicePixelRatio||1;c.width=Math.max(160,Math.floor(r.width*d));c.height=Math.max(120,Math.floor(r.height*d));}} function drawImageFit(ctx,c,im,bg='#06101d'){{const w=c.width,h=c.height;ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=bg;ctx.fillRect(0,0,w,h);if(!im.complete||!im.naturalWidth){{im.onload=()=>drawAll();return}}const s=Math.min(w/im.naturalWidth,h/im.naturalHeight),iw=im.naturalWidth*s,ih=im.naturalHeight*s;ctx.drawImage(im,(w-iw)/2,(h-ih)/2,iw,ih)}} function drawFrames(){{preload(frame);drawImageFit(rgbCtx,rgbCanvas,loadImage(rgbFrames[frame]));drawTrajectory();drawImageFit(tactileCtx,tactileCanvas,loadImage(tactileFrames[frame]),TACTILE_CANVAS_BG);}}
...
def main() -> None:
    ...
    web_hand_display_scale = max(float(args.hand_display_scale), 2.0)
    trajectory_data = build_trajectory_web_data(rows, all_points, hand_display_scale=web_hand_display_scale)
    html_path = write_html(session, chart_rows, trajectory_data, frame_times, fps=actual_fps, output_subdir=args.output_subdir)
    print(json.dumps({"html": str(html_path), "frames": len(rows), "rgb_frames": rgb_written, "traj_frames": str(outputs / args.output_subdir / "traj_frames"), "tactile_frames": tactile_written, "hand_display_rotate_deg": args.hand_display_rotate_deg, "hand_display_scale": args.hand_display_scale, "web_hand_display_scale": web_hand_display_scale, "scene_display_scale": args.scene_display_scale, "fps": actual_fps, "duration_sec": frame_times[-1] if frame_times else 0.0, "timebase": "rgb_stamp_ns", "output_subdir": args.output_subdir, "align_middle_vertical": args.align_middle_vertical}, ensure_ascii=False))
