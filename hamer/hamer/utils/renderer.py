...
class Renderer:
    ...
    def _get_offscreen_renderer(self, width, height):
        width = int(width)
        height = int(height)
        size = (width, height)
        if self._offscreen_renderer is not None and self._offscreen_size == size:
    ...
    def __call__(self,
        ...
        renderer = self._get_offscreen_renderer(image.shape[1], image.shape[0])
    ...
    def render_rgba(
        ...
        renderer = self._get_offscreen_renderer(render_res[0], render_res[1])
        # material = pyrender.MetallicRoughnessMaterial(
        #     metallicFactor=0.0,
        #     alphaMode='OPAQUE',
        #     baseColorFactor=(*mesh_base_color, 1.0))
